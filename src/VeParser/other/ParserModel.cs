...
namespace VeParser
{
    public class TokenParserData { public string ExpectedToken; }
    ...
    {
        static Dictionary<object, TokenParserData> models = new Dictionary<object, TokenParserData>();

        public static void SetExpectedToken(this object tokenParser, string value)
        {
            if (!models.ContainsKey(tokenParser))
                models.Add(key: tokenParser, value: new TokenParserData());
            models[tokenParser].ExpectedToken = value;
        }

        public static string GetExpectedToken(this object tokenParser)
        {
            if (!models.ContainsKey(tokenParser))
                return "{Unknown Token}";
            else
                return models[tokenParser].ExpectedToken;
        }
    }
}